      ...
      if (response.resultSets!.length > 1) {
        ...
        if (list.isNotEmpty) {
          company.value = Company(
              ...
              splashUrl: list[1]["SPLASHURL"] ?? "",
              slogan: list[1]["SLOGAN"] ?? "",
              address: list[1]["ADDRESS"] ?? "",
              lat: list[1]["LAT"] ?? "",
              long: list[1]["LONG"] ?? "",
              whatsAppPhone: list[1]["WHATSAPPPHONE"] ?? "",
              instagram: list[1]["INSTAGRAM"] ?? "",
              facebook: list[1]["FACEBOOK"] ?? "",
              twitter: list[1]["TWITTER"] ?? "",
              youtube: list[1]["YOUTUBE"] ?? "",
              spotify: list[1]["SPOTIFY"] ?? "",
              tiktok: list[1]["TIKTOK"] ?? "",
              vk: list[1]["VK"] ?? "",
              ok: list[1]["OK"] ?? "",
              pinterest: list[1]["PINTEREST"] ?? "",
              swarm: list[1]["SWARM"] ?? "",
              linkedin: list[1]["LINKEDIN"] ?? "",
              gdprlink: list[1]["GDPRLINK"] ?? "",
              emailpermissionlink: list[1]["EMAILPERMISSIONLINK"] ?? "");
        } else {
          Get.offNamed("/formError");
        }
      }
